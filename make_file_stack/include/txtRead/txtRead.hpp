...
public:
  ...
  void Replace_layer(int layer1,int layer2);             // 交换层
...
void txtRead::Replace_layer(int layer1,int layer2){
...
other:
    ...
    //交换master
    for(auto &i:master){
        i.data()[1]=i.data()[1]+(hight[layer1]-hight[2]);
    }

    //交换mental
    for(auto &i:mental){
        if(i.get_name()=="d1L" ||i.get_name()=="d1" || i.get_name()=="d2" || i.get_name()=="d2Env"){
            for(auto &j:i){
                j.data()[1]=j.data()[1]+(hight[layer2]-hight[4]);
            }
        }else if(i.get_name() =="c1Env" ||i.get_name()=="c2" || i.get_name()=="c2Env"){
            for(auto &j:i){
                j.data()[1]=j.data()[1]+(hight[layer1]-hight[2]);
            }
        }
    }
